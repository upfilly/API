...
<div class="container">
  ...
  ${data.brands.map((brand, index) => `
    <div class="brand-section ${index > 0 ? 'page-break' : ''}">
      <div class="brand-header">
        <div class="brand-title">${brand.brand_name}</div>
        <div class="brand-subtitle">
          ${brand.total_orders} orders • 
          Total Commission: $${brand.total_amount.toFixed(2)} • 
          Upfilly Fee (${brand.commission_override}%): $${brand.upfilly_fee.toFixed(2)}
        </div>
      </div>
      ...
      <table class="transaction-table">
            ...
            brand.commissions?.length
              ? brand.commissions.map(c => {
                  ...
                  <tr>
                    ...
                    <td class="order-id">${link.order_id || 'N/A'}</td>
                    <td class="period">${link.timestamp || `${data.month_name} ${data.year}`}</td>
        ...
        <tfoot>
          <tr>
            ...
            <td class="text-right">$${brand.commissions.reduce((s, c) => s + (c.affiliateLinkId?.price || 0), 0).toFixed(2)}</td>
            <td class="text-right">$${brand.total_amount.toFixed(2)}</td>
            <td class="text-right">$${brand.upfilly_fee.toFixed(2)}</td>
          </tr>
        </tfoot>
  ...
  <div class="footer">
    Generated by Upfilly Commission System • Report ID: ${reportNumber}
  </div>
</div>
</body>
</html>
