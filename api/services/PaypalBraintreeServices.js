...
exports.buy_subscription = async (options) => {
    let create_subscription = await stripe.subscriptions.create({
        ...
        trial_period_days: 7, // Set to 1 day instant of 7 day on 5 july for testing
        trial_settings: {
            end_behavior: {
...
exports.retrieve_subscrition = async (options) => {
    let retrieve_subscrition = await gateway.subscription.find(
        options.paypal_subscription_id
    );
...
exports.delete_subscription = async (options) => {
    const deleted = await gateway.subscription.cancel(
        options.paypal_subscription_id
    );
