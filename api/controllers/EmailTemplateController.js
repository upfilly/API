...
exports.create = async (req, res) => {
  try {
    let validation_result =
      await Validations.EmailTemplateValidation.addEmailTemplate(req, res);
    ...
    let isEmailTemplateExists = await EmailTemplate.findOne({
    ...
    return response.success(
      newTemplate,
      constants.EMAILTEMPLATE.CREATED,
      req,
      res
    );
  } catch (err) {
...
exports.read = async (req, res) => {
  try {
    ...
    return response.success(
      templates,
      constants.EMAILTEMPLATE.FETCHED,
      req,
      res
    );
  } catch (err) {
...
exports.update = async (req, res) => {
  try {
    let validation_result =
      await Validations.EmailTemplateValidation.editEmailTemplate(req, res);
    ...
    let templates = await EmailTemplate.updateOne(
      { id: id },
      { isDeleted: true }
    );
    await EmailTemplateAffiliate.update(
    ...
    return response.success(
      newTemplate,
      constants.EMAILTEMPLATE.UPDATED,
      req,
      res
    );
  } catch (err) {
...
exports.getUserEmailTemplate = async (req, res) => {
  try {
    ...
    if (search) {
      const cleanSearch =
        await Services.Utils.remove_special_char_exept_underscores(search);
      query.$or = [
          ...
          let: {
            campaignId: {
              $cond: {
                      ...
                      $eq: [
                        {
                          $type: {
                            $arrayElemAt: [
                              "$affiliate_campaign_links.campaign_id",
                              0,
                            ],
                          },
                        },
                ...
                then: {
                  $toObjectId: {
                    $arrayElemAt: ["$affiliate_campaign_links.campaign_id", 0],
                  },
                },
                else: "$$REMOVE",
              },
...
exports.affiliateCount = async (req, res) => {
  try {
    ...
    if (brandId) {
      ...
      // Only keep affiliates that exist and are not deleted
      const validAffiliateIds = validUsers.map((u) => u.id);
      ...
      const joinedAffiliates = validAffiliateIds.filter(
