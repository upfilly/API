...
module.exports = {
  ...
  register: async (req, res) => {
    try {
      ...
      let validation_result = await Validations.UserValidations.register(req, res);
      ...
      let get_user = await Users.findOne({ email: req.body.email.toLowerCase(), isDeleted: false });
      if (get_user) {
      ...
      if (add_user) {
        if (req.body.email) {
          let get_invites = await Invite.findOne({ email: req.body.email.toLowerCase(), isDeleted: false });
          if (get_invites) {
            let update_user = await Invite.updateOne({ id: get_invites.id }, { invite_status: "onboard" });
          }
        }
        ...
        return response.success(add_user, constants.user.SUCCESSFULLY_REGISTERED, req, res);
      }
    } catch (err) {
      console.log(err, "-------err");
  ...
  adminSignin: async (req, res) => {
    try {
      ...
      let validation_result = await Validations.UserValidations.adminSignin(req, res);
  ...
  changePassword: async function (req, res) {
    try {
      let validation_result = await Validations.UserValidations.changePassword(req, res);
      ...
      let update_user = await Users.updateOne({ id: req.identity.id }, { password: encryptedPassword });

      if (update_user) {
        return response.success(null, constants.user.PASSWORD_CHANGED, req, res);
      }
  ...
  userSignin: async (req, res) => {
    try {
      ...
      if (user.isVerified == 'N') {   // if user not verified then sent code to verify email
  ...
  verifyUserEmail: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.verifyUserEmail(req, res);
      ...
      if (get_user) {
        if (get_user.verificationCode) {
          if (get_user.verificationCode === verificationCode) {
            let update_user = await Users.updateOne({ id: id }, { isVerified: "Y", verificationCode: null, last_login: new Date() });
            let token = jwt.sign(
            ...
            return response.success(get_user, constants.user.SUCCESSFULLY_LOGGEDIN, req, res);
          }
  ...
  resendOtp: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.resendOtp(req, res);
      ...
      if (get_user) {
        let get_otp = await Services.CommonServices.generateOTP();
        if (get_otp) {
          let update_otp = await Users.updateOne({ id: get_user.id }, { verificationCode: get_otp, last_vc_updated_at: new Date() });
        ...
        throw constants.COMMON.SERVER_ERROR; k
      }
  ...
  getAllUsers: async (req, res) => {
    try {
      ...
      if (isTrusted) {
        query.isTrusted = isTrusted ? isTrusted === 'true' : true ? isTrusted : false;
      }

      if (isFeatured) {
        query.isFeatured = isFeatured ? isFeatured === 'true' : true ? isFeatured : false;
      }
  ...
  forgotPassword: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.forgotPassword(req, res);
      ...
      let verificationCode = await Services.CommonServices.generateVeificationCode();
      ...
      if (update_user) {
        ...
        return response.success(null, constants.user.VERIFICATION_SENT, req, res);
      }
      throw constants.COMMON.SERVER_ERROR;
  ...
  resetPassword: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.resetPassword(req, res);
      ...
      let update_user = await Users.updateOne({ id: user.id }, { password: encryptedPassword });
      if (update_user) {
        return response.success(null, constants.user.PASSWORD_CHANGED, req, res);
      }
  ...
  editProfile: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.editProfile(req, res);
      ...
      if ((req.body.lat && req.body.lng) && (req.body.lat != get_user.lat || req.body.lng != get_user.lng)) {
        req.body.lat = Number(req.body.lat);
        req.body.lng = Number(req.body.lng);
      ...
      if (permissions) {
        ...
        let get_user_permissions = await Permissions.findOne({ id: permissions.id, user_id: id });
        if (get_user_permissions) {
          permissions.updatedBy = req.identity.id;
          let update_permissions = await Permissions.updateOne({ id: permissions.id, user_id: id }, permissions);
          if (['team'].includes(req.identity.role)) {
            await Services.AuditTrial.create_audit_trial(req.identity.id, 'permissions', 'updated', update_permissions, get_user_permissions)
          }
        } else {
          ...
          if (valid_roles.includes(get_user.role)) {
            let create_permissions = await Permissions.create(permissions).fetch();
          }
        }
      }
      //---------------- Updating permissions -----------//
  ...
  addUser: async (req, res) => {
    try {
      ...
      if (createdByBrand) {
        var find_brand = await Users.findOne({ id: createdByBrand, isDeleted: false });
      ...
      if (newUser) {
        ...
        if (newUser && newUser.isSendActivationEmail && newUser.isSendActivationEmail == true) {
          let email_payload_new = {
            email: newUser.email,
            firstName: newUser.firstName,
            fullName: newUser.fullName,
        ...
        //   //------------------- Fetching data from creator db --------------//
        // }
        return response.success(newUser, constants.user.SUCCESSFULLY_REGISTERED, req, res);
      }
  ...
  userAutoLogin: async function (req, res) {
    try {
      ...
      update_user.subscribe_status = get_active_subscription ? 'active' : 'cancelled';
      // ---------- To Check User Have Any Active Subscriptions --------//
    ...
    try {
      ...
      if (role == "influencers") {
        worksheet.columns = [
          ...
          { header: "instagram link", key: "instagram_profile_link", width: 25 },
          ...
          { header: "pinterest link", key: "pinterest_profile_link", width: 25 },
        ...
        .upload(
          ...
          async (err, file) => {
            ...
            file.forEach(async (element, index) => {
              ...
              fileExt = typeArr[1];
              if (fileExt == 'vnd.openxmlformats-officedocument.spreadsheetml.sheet' || fileExt == "vnd.ms-excel") {
                            ...
                            if (counter == 0) {
                              ...
                              if (itm && itm.length > 0) {
                                ...
                                for await (let data of itm) {
                                  ...
                                  if (["full name", "Full Name", "Name"].includes(keyLabel)) { obj.firstName = data }   // new
                                  if (["email", "Email"].includes(keyLabel)) { obj.email = data }          // new
                                  if (["gender", "Gender"].includes(keyLabel)) { obj.gender = data }       // new
                                  if (["Country", "country"].includes(keyLabel)) { obj.country = data };
                                  if (["Cultures", "Culture", "culture", "cultures"].includes(keyLabel)) { obj.cultures = data }

                                  if (["Youtube Username", "youtube username", "youtube id", "Youtube Id"].includes(keyLabel)) { obj.youtube_username = data }
                                  if (["Youtube Profile Link", "youtube profile link", "youtube link", "Youtube Link"].includes(keyLabel)) { obj.youtube_profile_link = data }

                                  if (["Tiktok Username", "tiktok username", "tiktok id", "Tiktok Id"].includes(keyLabel)) { obj.tiktok_username = data }
                                  if (["Tiktok Profile Link", "tiktok profile link", "tiktok link", "Tiktok Link"].includes(keyLabel)) { obj.tiktok_profile_link = data }

                                  if (["Twitter Id", "twitter id"].includes(keyLabel)) { obj.twitter_username = data }
                                  if (["Twitter Profile Link", "twitter profile link", "Twitter Link", "twitter link"].includes(keyLabel)) { obj.twitter_profile_link = data }

                                  if (["Facebook Username", "facebook username", "Facebook Id", "facebook id"].includes(keyLabel)) { obj.facebook_username = data }
                                  if (["Facebook Profile Link", "facebook profile link", "Facebook Link", "facebook link"].includes(keyLabel)) { obj.facebook_profile_link = data }

                                  if (["Instagram Username", "instagram username", "Instagram Id", "instagram id"].includes(keyLabel)) { obj.instagram_username = data }
                                  if (["Instagram Profile Link", "instagram profile link", "Instagram Link", "instagram link"].includes(keyLabel)) { obj.instagram_profile_link = data }

                                  if (["Pinterest Username", "pinterest username", "Pinterest Id", "pinterest id"].includes(keyLabel)) { obj.pinterest_username = data }
                                  if (["Pinterest Profile Link", "pinterest profile link", "Pinterest Link", "pinterest link"].includes(keyLabel)) { obj.pinterest_profile_link = data }

                                  if (["LinkedIn Username", "linkedIn username", "Linkedin Username", "linkedin username", "Linkedin Id", "linkedin id"].includes(keyLabel)) { obj.linkedin_username = data }
                                  if (["LinkedIn Profile Link", "linkedIn profile link", "Linkedin Link", "linkedin link"].includes(keyLabel)) { obj.linkedin_profile_link = data }

                                  if (["Snapchat Username", "snapchat username", "Snapchat Id", "snapchat id"].includes(keyLabel)) { obj.snapchat_username = data }
                                  if (["Snapchat Profile Link", "snapchat profile link", "Snapchat Link", "snapchat link"].includes(keyLabel)) { obj.snapchat_profile_link = data }
                                ...
                                try {
                                  ...
                                  if (culture) {
                                    ...
                                    for await (let item of culture_arr) {
                                      let get_cultures = await CommonCategories.findOne({
                                  ...
                                  obj.social_media_platforms = await Services.UserServices.get_social_media_for_import(obj);
                                  if (obj.gender) {

                                    obj.gender = await Services.Utils.remove_whitespace_from_string(obj.gender);
                                    if (["M", "m", "Male", "male"].includes(obj.gender)) {
                                      obj.gender = "male";
                                    } else if (["F", "f", "Female", "female"].includes(obj.gender)) {
                                      obj.gender = "female";
                                    } else if (["T", "t", "Transgender", "transgender"].includes(obj.gender)) {
                                      obj.gender = "transgender";
                                    } else if (["O", "o", "others", "others"].includes(obj.gender)) {
                                      obj.gender = "others";
                                    } else {
                                      delete obj.gender;
                                    }
                                  ...
                                  if (obj) {
                                    ...
                                    try {
                                      ...
                                      if (obj.firstName && obj.email) {
                                        ...
                                        obj.email = await Services.Utils.remove_whitespace_from_string(obj.email);

                                        if (obj.youtube_username) {
                                          obj.youtube_username = Services.Utils.make_valid_social_media_username(obj.youtube_username);
                                        };

                                        if (obj.tiktok_username) {
                                          obj.tiktok_username = Services.Utils.make_valid_social_media_username(obj.tiktok_username);
                                        };

                                        if (obj.twitter_username) {
                                          obj.twitter_username = Services.Utils.make_valid_social_media_username(obj.twitter_username);
                                        };

                                        if (obj.facebook_username) {
                                          obj.facebook_username = Services.Utils.make_valid_social_media_username(obj.facebook_username);
                                        };

                                        if (obj.instagram_username) {
                                          obj.instagram_username = Services.Utils.make_valid_social_media_username(obj.instagram_username);
                                        };

                                        if (obj.pinterest_username) {
                                          obj.pinterest_username = Services.Utils.make_valid_social_media_username(obj.pinterest_username);
                                        };

                                        if (obj.linkedin_username) {
                                          obj.linkedin_username = Services.Utils.make_valid_social_media_username(obj.linkedin_username);
                                        };

                                        if (obj.snapchat_username) {
                                          obj.snapchat_username = Services.Utils.make_valid_social_media_username(obj.snapchat_username);
                                        };
                                        ...
                                        query.isDeleted = false;
                                        let get_users = await Users.findOne(query);

                                        if (!get_users) {
                                          obj.addedBy = req.identity.id;
                                          obj.role = "influencers";
                                          obj.fullName = obj.firstName;
                                          let create_users = await Users.create(obj).fetch();
                                          if (create_users) {
                                            if (["influencers"].includes(create_users.role)) {
                                              influencers_ids_arr.push(create_users.id)
                                            }
                                          }
                                          imported++
                                        }
        ...
        .upload(
          ...
          async (err, file) => {
            ...
            file.forEach(async (element, index) => {
              ...
              if (
                fileExt == 'vnd.openxmlformats-officedocument.spreadsheetml.sheet' || fileExt == "vnd.ms-excel"
              ) {
                let name = `${file[index].fd.split('/csv')[1]}`
                fs.readFile(file[index].fd, async (err, data) => {
                            ...
                            if (counter == 0) {
                              ...
                              if (itm && itm.length > 0) {
                                ...
                                for await (let data of itm) {
                                  ...
                                  if (keyLabel == 'Brand Name') { obj.fullName = data }
                                  if (keyLabel == 'Email') { obj.email = data }
                                  if (keyLabel == 'DialCode') { obj.dialCode = data }
                                  if (keyLabel == 'MobileNo') { obj.mobileNo = data }
                                ...
                                try {
                                  ...
                                  if (obj) {
                                    ...
                                    if (obj.dialCode && obj.mobileNo) {
                                      let get_country_data = await Services.CommonServices.get_country_code_with_dial_code(obj.dialCode);

                                      if (get_country_data) {
                                        obj.phone = {
                                          countryCode: get_country_data.a2,
                                          dialCode: obj.dialCode ? `+${obj.dialCode}` : "",
                                          number: `${obj.mobileNo}`,
                                        }
                                      }
                                    }

                                    obj.dialCode = obj.dialCode ? `+${obj.dialCode}` : "";
                                    obj.email = obj.email.toLowerCase();
                                    try {
                                      ...
                                      if (obj.email && obj.fullName) {
                                        ...
                                        if (!get_users) {
                                          ...
                                          obj.isVerified = 'Y';
                                          obj.my_code = Services.Referral.generate_referal_code();
                                          let create_users = await Users.create(obj).fetch();
                                          if (create_users) {
                                            let email_roles = ['brand'];
                                            if (email_roles.includes(create_users.role)) {
                                              let email_payload = {
  ...
  updateBasicDetails: async (req, res) => {
    try {
      let validation_result = await Validations.UserValidations.updateBasicDetails(req, res);
  ...
  getUserByEmail: async (req, res) => {
    try {
      ...
      let get_user = await Users.findOne({
        where: { email: req.query.email.toLowerCase(), isDeleted: false, role: { in: ["influencers"] } },
        select: [
          'email',
          'role',
          'firstName',
  ...
  userSocialLogin: async (req, res) => {
    try {
      ...
      let validation_result = await Validations.UserValidations.userSocialLogin(req, res);
      ...
      let new_date = new Date();
      let { email, role, facebook_auth_id, google_auth_id, device_token } = req.body;
      let query = { isDeleted: false };
      if (email) {
      ...
      if (newUser) {
        ...
        return response.success(newUser, constants.user.SUCCESSFULLY_LOGGEDIN, req, res);
      }
      // -------------- If user not found then creating users -----------//
  ...
  deleteUserWithoutAuth: async (req, res) => {
    try {
      ...
      const delete_user = await Users.updateOne({ id: id }, { isDeleted: true });
      if (delete_user) {
  ...
  googleLogin: async (req, res) => {
    try {
      let oAuth2Client = new OAuth2Client({
        clientId: constant.GOOGLE_LOGIN.GOOGLE_CLIENT_ID,
        clientSecret: constant.GOOGLE_LOGIN.GOOGLE_CLIENT_SECRET,
        redirectUri: req.header('Referer') ? req.header('Referer') + "login" : constant.GOOGLE_LOGIN.GOOGLE_LOGIN_REDIRECT,
      });
