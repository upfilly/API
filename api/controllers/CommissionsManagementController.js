...
exports.addCommission = async (req, res) => {
    try {
        let validation_result = await Validations.CommissionValidation.addCommssion(req, res);
        ...
        if (affiliate_group && affiliate_group != null) {
            var get_affiliate_group = await AffiliateManagement.findOne({ id: affiliate_group, isDeleted: false });
            if (!get_affiliate_group) {
                throw constants.COMMISSION.INVALID_AFFILIATE_GROUP
            }
            const existedCommission = await CommissionsManagement.findOne({ affiliate_group: get_affiliate_group.id, addedBy: req.identity.id, event_type: event_type, isDeleted: false });
...
exports.editCommssion = async (req, res) => {
    try {
        let validation_result = await Validations.CommissionValidation.editCommission(req, res);
        ...
        let get_commission = await CommissionsManagement.findOne({ id: id, isDeleted: false });
        if (!get_commission) {
            throw constants.COMMISSION.INVALID_ID;
        }

        req.body.updatedBy = req.identity.id;
        let edit_affiliate_group = await CommissionsManagement.updateOne({ id: id }, req.body);
    ...
    try {
        ...
        let pipeline = [
            ...
            // },
            {
                $lookup: {
                    from: "users",
                    let: { affiliate_group: "$affiliate_group", isDeleted: false, addedBy: "$addedBy" },
                    pipeline: [
                        {
                            $match: {
                                $expr: {
                                    $and: [
                                        { $and: [{ $eq: ["$affiliate_group", "$$affiliate_group"] }, { $eq: ["$addedBy", "$$addedBy"] }, { $eq: ["$isDeleted", "$$isDeleted"] }] },
                                        { $and: [{ $ne: ["$affiliate_group", null] }, { $eq: ["$isDeleted", "$$isDeleted"] }] }
                                    ]
                                }
                            }
                        }
        ...
        // Pipeline Stages
        db.collection('commissionsmanagement').aggregate(pipeline).toArray((err, totalresult) => {
            pipeline.push({
                $skip: Number(skipNo)
            });
            pipeline.push({
            ...
            db.collection("commissionsmanagement").aggregate(pipeline).toArray((err, result) => {
                let resData = {
    ...
    try {
        ...
        if (get_commission) {
            if (get_commission.plan_id) {
                let get_plan_details = await SubscriptionPlans.findOne({ id: get_commission.plan_id });
                if (get_plan_details) {
                    get_commission.plan_name = get_plan_details.name;
                }
            }
            if (get_commission.affiliate_group) {
                let get_group_details = await AffiliateManagement.findOne({ id: get_commission.affiliate_group });
                if (get_group_details) {
                    get_commission.group_name = get_group_details.group_name;
                }
            }
            return response.success(get_commission, constants.COMMISSION.FETCHED, req, res);
        }
        throw constants.COMMISSION.INVALID_ID;
