...
exports.addCampaign = async (req, res) => {
  try {
    ...
    // return;
    let validation_result = await Validations.CampaignValidations.addCampaign(
      req,
      res,
    );
    ...
    if (!get_brand) {
      return response.failed(
        null,
        constants.CAMPAIGN.INVALID_BRAND_ID,
        req,
        res,
      );
    }
    ...
    if (req.body.isDefault) {
      ...
      await Campaign.update({ brand_id: brand_id, isDefault: true }).set({
        isDefault: false,
      });
    } else {
    ...
    if (req.body.affiliate_id && req.body.access_type === "private") {
      ...
      affiliate_id_list = await Users.find({
        role: "affiliate",
        isDeleted: false,
      });
    ...
    if (add_campaign) {
      ...
      for (let current_affiliate_id of affiliate_id_list) {
        ...
        let create_notification =
          await Notifications.create(notification_payload).fetch();
      }
      //------------------------Create Logs here -------------------------------
      if (add_campaign) {
...
exports.editCampaign = async (req, res) => {
  try {
    ...
    let validation_result = await Validations.CampaignValidations.editCampaign(
      req,
      res,
    );
...
exports.getAllCampaignRequestsForAffiliate = async (req, res) => {
  try {
    ...
    if (loggedInUser.addedBy) {
      ...
      if (get_account_manager_detail && get_account_manager_detail.role) {
        ...
        if (
          ...
          isPermissionExists = await Permissions.findOne({
            role: loggedInUser.role,
          });
        } else {
          isPermissionExists = await Permissions.findOne({
    ...
    if (search && search.trim()) {
      ...
      new_query.$or = [{ campaign_name: { $regex: search, $options: "i" } }];
    }
    ...
    if (sub_child_category) {
      sub_child_category =
        await Services.Utils.string_to_array(sub_child_category);
      new_query.sub_child_category = { $in: sub_child_category };
    }
    if (region) {
      region = await Services.Utils.string_to_array(region);
...
exports.getAllCampaignsForBrand = async (req, res) => {
  try {
    ...
    if (sub_category) {
      // query.sub_category_id = new ObjectId(sub_category_id);
      sub_category =
        await Services.Utils.string_ids_toObjectIds_array(sub_category);
      query.sub_category = { $in: sub_category };
    }
    if (category) {
      query.category =
        await Services.Utils.string_ids_toObjectIds_array(category);
    ...
    if (affiliate_ids) {
      affiliateIdArray = affiliate_ids
        .split(",")
    ...
    let pipeline = [
      ...
      {
        $match: {
          ...(affiliateIdArray.length > 0
            ? { filtered_affiliates: { $ne: [] } }
            : {}),
        },
      },
      {
...
exports.getAllCampaignsForAffiliate = async (req, res) => {
  try {
    if (req.query.campaign) {
      ...
      return response.success(
        data,
        constants.CAMPAIGN.AFFILIATE_FETCH_IN_CAMPAIGN,
        req,
        res,
      );
    } else {
  ...
  try {
    ...
    if (get_campaign.category && get_campaign.category.length > 0) {
      ...
      for await (let cat of get_campaign.category) {
        ...
        let data = await CommonCategories.findOne({ id: cat.id || cat }).select(
          ["id", "name"],
        );
    ...
    if (
      get_campaign.sub_child_category &&
      get_campaign.sub_child_category.length > 0
    ) {
      let sub_child_category = [];
      for await (let cat of get_campaign.sub_child_category) {
        let data = await SubChildCategory.findOne({ id: cat.id || cat }).select(
          ["id", "name"],
        );
    ...
    let listOfAffiliates = await BrandAffiliateAssociation.find({
      where: {
        campaign_id: get_campaign.id,
        brand_id: get_campaign.brand_id.id,
        isDeleted: false,
        isActive: true,
      },
      select: ["affiliate_id", "status", "reason", "campaign_link"],
    }).populate("affiliate_id");
...
exports.changeCampaignStatus = async (req, res) => {
  try {
    ...
    let validation_result =
      await Validations.CampaignValidations.changeCampaignStatus(req, res);
    ...
    let get_campaign = await BrandAffiliateAssociation.findOne({
      id: id,
      isDeleted: false,
    ...
    if (get_campaign) {
      ...
      if (req.body.status === "accepted") {
        ...
        await BrandAffiliateAssociation.updateOne({ id: id }).set({
          status: req.body.status,
          isActive: true,
        });
      } else if (req.body.status === "rejected") {
        await BrandAffiliateAssociation.updateOne({ id: get_campaign.id }).set({
          status: "rejected",
        });
      } else {
        return response.failed(
          null,
          constants.CAMPAIGN.INVALID_STATUS,
          req,
          res,
        );
      }
      ...
      let brandDetail = await Users.findOne({
        id: get_campaign.brand_id,
        isDeleted: false,
      });
      let create_notification =
        await Notifications.create(notification_payload).fetch();
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    let brandAffiliateAssociation = await BrandAffiliateAssociation.find({
      campaign_id: id,
      isActive: true,
      status: "accepted",
    });
    if (brandAffiliateAssociation && brandAffiliateAssociation.length > 0) {
      return response.failed(
        null,
        constants.CAMPAIGN.NOT_ALLOWED_AFFS_EXIST,
        req,
        res,
      );
    }
    ...
    if (campaign.isDefault) {
      let campaignsForBrand = await Campaign.find({
        brand_id: campaign.brand_id,
        isDeleted: false,
      }).sort({ updatedAt: -1 });
      if (campaignsForBrand && campaignsForBrand.length > 0) {
        await Campaign.updateOne({ id: campaignsForBrand[0].id }).set({
          isDefault: true,
        });
      }
    }
    const update_campaign = await Campaign.updateOne({ id: id }).set({
      isDefault: false,
      isDeleted: true,
      updatedBy: req.identity.id,
    });
    await BrandAffiliateAssociation.update({ campaign_id: id }).set({
      isDeleted: true,
    });
    if (update_campaign) {
...
exports.removeAffiliate = async (req, res) => {
  try {
    ...
    const association = await BrandAffiliateAssociation.findOne({
      brand_id: brand_id,
...
exports.campaignAffiliates = async (req, res) => {
  try {
    ...
    const total = await BrandAffiliateAssociation.count({
      brand_id: brand,
      campaign_id: campaign,
      status: "accepted",
    });

    const affiliates = await BrandAffiliateAssociation.find({
      brand_id: brand,
      campaign_id: campaign,
      status: "accepted",
    })
    ...
    return response.success(
      data,
      "Affiliates fetched with pagination",
      req,
      res,
    );
  } catch (error) {
