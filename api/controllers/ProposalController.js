...
module.exports = {
    ...
    addproposal: async (req, res) => {
        try {
            let validation_result = await Validations.ProposalValidations.addProposal(req, res);
    ...
    getAllProposals: async (req, res) => {
        try {
            ...
            // Pipeline Stages
            db.collection('proposal').aggregate(pipeline).toArray((err, totalresult) => {
                pipeline.push({
                    $skip: Number(skipNo)
                });
                pipeline.push({
                ...
                db.collection("proposal").aggregate(pipeline).toArray((err, result) => {
                    let resData = {
    ...
    changeStatus: async (req, res) => {
        try {
            ...
            let validation_result = await Validations.ProposalValidations.changeCampaignStatus(req, res);
            if (validation_result && !validation_result.success) {
                throw validation_result.message
            }
            ...
            if (req.body.status == "accepted" && ['accepted'].includes(get_proposal.status)) {
            ...
            let update_status = await Proposal.updateOne({ id: req.body.id }, req.body);
