        ...
        if (findAccount) {
          const updateObject = {
            ...
            bank_name:
              eventObject.external_accounts?.data[0]?.bank_name || "",
            country:
              eventObject.external_accounts?.data[0]?.country || "",
            currency:
              eventObject.external_accounts?.data[0]?.currency || "",
            accountStatus:
              eventObject.external_accounts?.data[0]?.status || "",
            routingNumber:
              eventObject.external_accounts?.data[0]?.routing_number || "",
  ...
  transferPayment: async (req, res) => {
    try {
      ...
      for await (let itm of affiliateLinkIds) {
        ...
        if (
          !get_campain_from_affiliations ||
          !get_campain_from_affiliations.campaign_id
        ) {
        ...
        try {
          ...
          if (paid) {
            ...
            const outputPath = path.join(invoicesDir, filename);
             ...
             const country = accountDetails.country || userDetail.country || 'N/A';
...
const invoice_itm_html = (payload) => {
  ...
  const invoice_itm_html = (payload) => {
  const { commission, fullName, stripeTransferId, invoiceDate, country } = payload; // Added country
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* NEW: Country info - only addition */
