...
exports.addInvite = async (req, res) => {
    try {
        ...
        let validation_result = await Validations.InviteValidation.addInvite(req, res);
        ...
        let get_invite = await Invite.findOne({ email: req.body.email, isDeleted: false });
        if (get_invite) {
    ...
    try {
        ...
        if (get_invite) {
            if (get_invite.addedBy) {
                let get_brand = await Users.findOne({ id: get_invite.addedBy, isDeleted: false });
...
exports.getAllInvite = async (req, res) => {
    try {
        ...
        db.collection('invite').aggregate(pipeline).toArray((err, totalresult) => {
            pipeline.push({
                $skip: Number(skipNo)
            });
            pipeline.push({
            ...
            db.collection("invite").aggregate(pipeline).toArray((err, result) => {
                let resData = {
                    data: result ? result : [],
