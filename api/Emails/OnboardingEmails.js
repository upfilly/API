...
exports.send_mail_to_brand = async (options) => {
    ...
    let affiliate_name = options.affiliate_name
...
border: none;
color: #fff;
padding: 10px 20px;border-radius: 30px;cursor: pointer;  "href="${credentials.FRONT_WEB_URL}/login">Go to our website</a>
</div>
...
`
    ...
    SmtpController.sendEmail(email, 'Untrack Sales', message)
};

exports.send_request_mail_to_brand = async (options) => {
    // console.log(options, "=options");
    let email = options.email
    let name = options.name
    let affiliate_name = options.affiliate_name
...
`
    ...
    SmtpController.sendEmail(email, 'Request from affiliate', message)
};
...
exports.change_status_affiliateInvite = async (options) => {
    ...
    let matter;
    if (status == 'rejected') {
        matter = `Regret to inform you that the request you have sent to is rejected.
         <br><br>
         <b> Reason : </b>${reason} <br><br>Better luck for the next time. Thanks for the business.`
    } else {
        matter = `Congratulations,<br> The request you have sent has been accepted. We wish good luck for the future venture. Thanks for your business.`
    }
    message = '';
    message += `
    ...
    <div style="width:600px;margin: auto;margin-top: 2rem;box-shadow: 0px 0px 20px -15px #000;position: relative;">
        <div style="text-align: center;">
            ...
            text-align: center;  display: -webkit-flex; border: 5px solid #00BAFF; background: #fff; margin: auto;  border-radius: 50%; display: flex; justify-content: center;align-items: center;">
                <div>
                         <img src="${credentials.BACK_WEB_URL}/images/upfilly.png"style="width:115px; height: 40px; object-fit: contain;">
                 <h1 style="  margin-bottom: 0px;  margin-top: 10px; font-size: 18px;"><span style="font-weight: 400;color:#373737;">Hi </span></h1>
                 <p style="margin-top: 0px;    font-size: 14px; color:#373737; margin-bottom: 0;">This is an notification email to let you know about your Affiliateinvite status.
                 </p>
                 </div>
                 </div>
                 </div>
 ...
 `
    ...
    SmtpController.sendEmail(email, 'Affiliate Invite request', message)
};


exports.changeRequestStatus = async (options) => {
    var status = options.status;
    var reason = options.reason;
    var get_user = await Users.findOne({ id: options.id })
    var email = get_user.email;

    let get_admin = await Users.find({ role: "admin", isDeleted: false });
     ...
     <div style="width:600px;margin: auto;margin-top: 2rem;box-shadow: 0px 0px 20px -15px #000;position: relative;">
         <div style="text-align: center;">
             ...
             text-align: center;  display: -webkit-flex; border: 5px solid #00BAFF; background: #fff; margin: auto;  border-radius: 50%; display: flex; justify-content: center;align-items: center;">
                 <div>
                          <img src="${credentials.BACK_WEB_URL}/images/upfilly.png"style="width:115px; height: 40px; object-fit: contain;">
                  <h1 style="  margin-bottom: 0px;  margin-top: 10px; font-size: 18px;"><span style="font-weight: 400;color:#373737;">Hi </span>${get_user.fullName},</h1>
                  <p style="margin-top: 0px;    font-size: 14px; color:#373737; margin-bottom: 0;">This is an notification email to let you know about account registration request.
                  </p>
                  </div>
                  </div>
                  </div>
  ...
  `
    ...
    SmtpController.sendEmail(email, 'Account Registration Request', message)
};
