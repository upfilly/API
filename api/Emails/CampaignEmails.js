...
exports.changeStatus = async (options) => {
  ...
  let matter;
  if (status == 'rejected') {
    matter = `Regret to inform you that the request you have sent to <b>${get_affiliate.fullName}</b> for the campaign <b>${campaignName}</b> is rejected.
      <br><br>
      <b> Reason : </b>${reason} <br><br>Better luck for the next time. Thanks for the business.`;
  } else {
    matter = `Congratulations,<br> The request you have sent to <b>${get_affiliate.fullName}</b> for the campaign <b>${campaignName}</b> has been accepted. We wish good luck for the future venture. Thanks for your business.`;
  }
      ...
      <div style="text-align: center;">
          ...
          text-align: center;  display: -webkit-flex; border: 5px solid #00BAFF; background: #fff; margin: auto;  border-radius: 50%; display: flex; justify-content: center;align-items: center;">
              <div>
                       <img src="${credentials.BACK_WEB_URL}/images/upfilly.png"style="width:115px; height: 40px; object-fit: contain;">
               <h1 style="  margin-bottom: 0px;  margin-top: 10px; font-size: 18px;"><span style="font-weight: 400;color:#373737;">Hi </span>${get_brand.fullName},</h1>
               <p style="margin-top: 0px;    font-size: 14px; color:#373737; margin-bottom: 0;">This is a notification email to let you know about your campaign request.
               </p>
               </div>
               </div>
               </div>
...
exports.changeRequestStatus = async (options) => {
   ...
   var get_affiliate = await Users.findOne({ id: options.affiliate_id })
   var get_brand = await Users.findOne({ id: options.brand_id })
   var email = get_affiliate.email;

   // let name_variable = (await Services.Utils.get_first_letter_from_each_word(get_user ? get_user.fullName : ""));
   let matter;
   if (status == 'rejected') {
      matter = `Regret to inform you that the request you have sent to ${get_brand.fullName} is rejected.
        <br><br>
        <b> Reason : </b>${reason} <br><br>Better luck for the next time. Thanks for the business.`
   } else {
      matter = `Congratulations,<br> The request you have sent to ${get_brand.fullName} has been accepted. We wish good luck for the future venture. Thanks for your business.`
   }
   message = '';
   message += `
   <body style="font-family: sans-serif;">
   ...
   <div style="width:600px;margin: auto;margin-top: 2rem;box-shadow: 0px 0px 20px -15px #000;position: relative;">
       <div style="text-align: center;">
           ...
           text-align: center;  display: -webkit-flex; border: 5px solid #00BAFF; background: #fff; margin: auto;  border-radius: 50%; display: flex; justify-content: center;align-items: center;">
               <div>
                        <img src="${credentials.BACK_WEB_URL}/images/upfilly.png"style="width:115px; height: 40px; object-fit: contain;">
                <h1 style="  margin-bottom: 0px;  margin-top: 10px; font-size: 18px;"><span style="font-weight: 400;color:#373737;">Hi </span>${get_affiliate.fullName},</h1>
                <p style="margin-top: 0px;    font-size: 14px; color:#373737; margin-bottom: 0;">This is an notification email to let you know about campaign request.
                </p>
                </div>
                </div>
